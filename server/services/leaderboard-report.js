...
const BADGE_META = {
  ...
  oracle:        { emoji: '🧠', name: 'האורקל', desc: 'מספר הפגיעות (מדויק או קרוב) הגבוה ביותר' },
  goal_machine:  { emoji: '🌿', name: 'מכונת אצוות', desc: 'מספר הניחושים הקרובים (סטייה של רמה אחת) הגבוה ביותר' },
  sharpshooter:  { emoji: '🎯', name: 'צלף', desc: 'הדיוק הגבוה ביותר (נקודות לדירוג)' },
  streak:        { emoji: '🔥', name: 'רצף חם', desc: 'הרצף הפעיל הארוך ביותר של דירוגים מזכי-נקודות' },
  perfectionist: { emoji: '💎', name: 'פרפקציוניסט', desc: 'היחס הגבוה ביותר של ניחושים מדויקים' },
  dedicated:     { emoji: '🦅', name: 'החרוץ', desc: 'מספר הדירוגים הרב ביותר שהוגשו' },
  centurion:     { emoji: '💯', name: 'מועדון ה-100', desc: '100 נקודות ומעלה' },
  prophet:       { emoji: '⭐', name: 'הנביא', desc: 'בונוס מיוחד (שמור לעתיד)' }
};
...
function buildLeaderboardSvg(rows, dateLabel, limit) {
  ...
  parts.push(`<text x="${W - 30}" y="80" font-family="${TEXT_FONT_STACK}" font-size="18" fill="#cfe8d8" text-anchor="end">הצבעת אצווה · ${xmlEscape(dateLabel)}</text>`);
...
async function getUserRecipients(board, audience) {
  ...
  if (mode === 'top10') {
    ...
    return db.query(`
      ...
      FROM users
      WHERE is_admin = 0 AND is_guest = 0 AND id NOT IN (SELECT user_id FROM sim_users)
  ...
  const rows = await db.query(`
    SELECT id, email, name
    FROM users
    WHERE id IN (${placeholders}) AND id NOT IN (SELECT user_id FROM sim_users)
  `, ids);
    ...
    try {
      await transporter.sendMail({
        ...
        html: buildLeaderboardEmailHtml({
          ...
          intro: 'מצורפת תמונת <strong>טבלת המצטיינים</strong> של 10 המובילים, כולל <strong>תגי הישג</strong>',
          showImage: true,
          footerText: 'לכניסה לאתר - הצבעת אצווה'
        }),
        attachments: [{ filename, content: png, contentType: 'image/png', cid: 'leaderboard' }]
      });
...
module.exports = {
  ...
  buildLeaderboardSvg,
  // עזרים משותפים לשליחת דוא״ל (בשימוש גם ב-activity-report)
  readSettingsMap,
  isTruthySetting,
