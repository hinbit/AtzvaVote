...
// מספר חברים מחזיקים סט משותף של ניחושי רמת הצלחה (1-5) לאצוות. מנהיג הקבוצה קובע את הניחושים.
...
// ────────── קביעת/עדכון ניחוש משותף (מנהיג בלבד) ──────────
router.post('/:id/bets', auth(), async (req, res) => {
  try {
    const groupId = Number(req.params.id);
    const batchId = Number(req.body?.batch_id);
    const guessLevel = Number(req.body?.guess_level);
    if (!Number.isInteger(guessLevel) || guessLevel < 1 || guessLevel > 5) {
      return res.status(400).json({ error: 'ניחוש לא תקין (רמה 1-5)' });
    }
    const group = await db.one('SELECT * FROM guess_groups WHERE id = ?', [groupId]);
    if (!group) return res.status(404).json({ error: 'קבוצה לא נמצאה' });
    if (!(await canManage(group, req))) {
      return res.status(403).json({ error: 'רק מנהיג הקבוצה קובע את הניחושים' });
    }
    ...
    if (batch.status !== 'active') {
      return res.status(403).json({ error: 'הניחושים פתוחים רק על אצווה פעילה' });
    }

    await db.run(`
      INSERT INTO guess_group_bets (group_id, batch_id, guess_level)
      VALUES (?, ?, ?)
      ON DUPLICATE KEY UPDATE guess_level = VALUES(guess_level), points = 0, updated_at = CURRENT_TIMESTAMP
    `, [groupId, batchId, guessLevel]);
    res.json({ ok: true });
  } catch (e) {
...
// ────────── הסרת ניחוש (כל עוד האצווה פעילה) ──────────
router.delete('/:id/bets/:batchId', auth(), async (req, res) => {
  try {
    const groupId = Number(req.params.id);
    ...
    if (batch && batch.status !== 'active') {
      return res.status(403).json({ error: 'מאוחר מדי — האצווה כבר אינה פעילה' });
    }
    await db.run('DELETE FROM guess_group_bets WHERE group_id = ? AND batch_id = ?', [groupId, batchId]);
    res.json({ ok: true });
  } catch (e) {
