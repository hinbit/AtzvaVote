...
// דירוג דמו דטרמיניסטי (1-5) לפי המשתמש והאצווה
function buildDemoRating(batch, userIndex) {
  const seed = hashInt(`${userIndex}:${batch.id}`);
  return (seed % 5) + 1;
}

// מיפוי מחלקה → תחנת דירוג (הגדרה department_stages, JSON)
async function readDepartmentStages(tx = db) {
  const row = await tx.one('SELECT `value` FROM settings WHERE `key` = ?', ['department_stages']);
  if (!row?.value) return {};
  try {
...
async function createBackupArchive(sqlBuffer) {
  const tmpRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'atzvavote-backup-'));
...
async function importBackupArchive(buffer) {
  const tmpRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'atzvavote-import-'));
...
// מנהל מערכת מלא (admin) ניגש להכל. מנהל-משנה (manager) מוגבל לניהול משתמשים ואצוות בלבד.
const MANAGER_GET = [/^\/overview$/, /^\/departments$/];
const MANAGER_ANY = [/^\/users(\/.*)?$/, /^\/batches(\/.*)?$/, /^\/recalculate$/];
...
router.get('/overview', async (req, res) => {
  try {
    ...
    const ratings    = await db.one('SELECT COUNT(*) AS n FROM batch_ratings');
    const batchesC   = await db.one('SELECT COUNT(*) AS n FROM batches');
    const finished   = await db.one("SELECT COUNT(*) AS n FROM batches WHERE status = 'finished'");
    res.json({
      users:    users.n,
      ratings:  ratings.n,
      batches:  batchesC.n,
  ...
  try {
    const rows = await db.query(`
      SELECT u.id, u.email, u.name, u.phone_number, u.department, u.is_admin, u.can_guess_groups, u.publish_prediction, u.gender, u.role, u.created_at, u.last_login_at,
        (SELECT COUNT(*) FROM batch_ratings WHERE user_id = u.id) AS num_predictions,
...
const MISSING_HEADERS = ['שם', 'טלפון', 'מספר נקודות', 'מיקום נוכחי', 'דירוגים שמולאו'];

// בונה את שורות דוח החוסרים (משתמשים שחסר להם לפחות 40% מהדירוגים לאצוות הפעילות הפתוחות)
async function buildMissingRatingsRows() {
  const active = await db.query(`
    SELECT id FROM batches
    WHERE status = 'active' AND rating_open = 1
  ...
  const rows = await db.query(`
    SELECT u.id, u.name, u.phone_number,
      (SELECT COUNT(*) FROM batch_ratings r
         WHERE r.user_id = u.id AND r.batch_id IN (${placeholders})) AS filled
    FROM users u
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      'דירוגים שמולאו': `${r.filled}/${ids.length}`
    };
  });
}
...
// ייצוא XLS של משתמשים (שם, טלפון) שחסרים להם לפחות 40% מהדירוגים לאצוות הפעילות
// ?format=xlsx|csv.
router.get('/users/export-missing', async (req, res) => {
  try {
    const format = String(req.query.format || 'xlsx').toLowerCase();
    const data = await buildMissingRatingsRows();
    const ws = data.length
      ? XLSX.utils.json_to_sheet(data, { header: MISSING_HEADERS })
      : XLSX.utils.aoa_to_sheet([MISSING_HEADERS]);
    const baseName = `missing-ratings-${new Date().toISOString().slice(0, 10)}`;
    ...
    const buf = rowsToXlsxBuffer(data, MISSING_HEADERS, 'MissingRatings');
...
// לשימוש כשליחת וואטסאפ דרך TmpSender. ?only_phone=05... (מצב בדיקה — שורה אחת)
router.post('/users/export-missing-link', async (req, res) => {
  try {
    const onlyPhone = String(req.query.only_phone || '').replace(/[^\d+]/g, '');

    let data = await buildMissingRatingsRows();
    if (onlyPhone) {
      ...
      data = [found || { 'שם': 'בדיקה', 'טלפון': onlyPhone, 'מספר נקודות': 0, 'מיקום נוכחי': '', 'דירוגים שמולאו': '0/0' }];
    }
...
const ACTIVITY_HEADERS = ['שם', 'טלפון', 'מחלקה', 'מספר דירוגים', 'נכונים', '% נכונים', 'מספר נקודות', 'מיקום נוכחי'];

// בונה את שורות הייצוא לפי מחלקה/כמות-דירוגים/אחוז-נכונים מתוך פרמטרי השאילתה
async function buildActivityRows(q) {
  ...
  // יותר מ-X% דירוגים נכונים (נקודות>0) מבין האצוות שנפתרו
  const finishedRow = await db.one("SELECT COUNT(*) AS c FROM batches WHERE status = 'finished'");
  ...
  if (Number.isFinite(minPct)) {
    ...
    else havingParts.push('1 = 0'); // אין אצוות שנפתרו — אף אחד לא עובר סף אחוזים
  }

  const rows = await db.query(`
    SELECT u.id, u.name, u.phone_number, u.department,
      (SELECT COUNT(*) FROM batch_ratings r WHERE r.user_id = u.id) AS cnt,
      (SELECT COUNT(*) FROM batch_ratings r
         JOIN batches b ON b.id = r.batch_id
         WHERE r.user_id = u.id AND b.status = 'finished' AND r.points > 0) AS correct
    FROM users u
...
// ייצוא XLS של משתמשים (שם, טלפון, מחלקה) לפי מחלקה/כמות-דירוגים/אחוז-נכונים. ?format=xlsx|csv
router.get('/users/export-by-activity', async (req, res) => {
  try {
    ...
    const baseName = `users-by-ratings-${new Date().toISOString().slice(0, 10)}`;
    ...
    const buf = rowsToXlsxBuffer(data, ACTIVITY_HEADERS, 'UsersByRatings');
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="${baseName}.xlsx"`);
    return res.send(buf);
  } catch (e) {
...
router.post('/users/export-by-activity-link', async (req, res) => {
  try {
    ...
    if (onlyPhone) {
      const found = data.find((r) => String(r['טלפון']).replace(/[^\d+]/g, '') === onlyPhone);
      data = [found || { 'שם': 'בדיקה', 'טלפון': onlyPhone, 'מחלקה': '', 'מספר דירוגים': 0, 'נכונים': 0, '% נכונים': 0, 'מספר נקודות': 0, 'מיקום נוכחי': '' }];
    }
    const buf = rowsToXlsxBuffer(data, ACTIVITY_HEADERS, 'UsersByRatings');
...
// יצירת 10 משתמשי דמו עם דירוגי אצוות מלאים
router.post('/users/demo', async (req, res) => {
  try {
    const batches = await db.query("SELECT id FROM batches WHERE status <> 'cancelled' ORDER BY id ASC");
    const departments = await readDepartments();
    const generated = [];
    let ratingsInserted = 0;

    await db.tx(async (t) => {
      const stageMap = await readDepartmentStages(t);
      ...
      for (let i = 1; i <= 10; i += 1) {
        ...
        await t.run('DELETE FROM batch_ratings WHERE user_id = ?', [user.id]);
        await t.run('DELETE FROM rating_history WHERE user_id = ?', [user.id]);

        // תחנת הדירוג לפי מיפוי המחלקה (department_stages); ללא מיפוי — ברירת מחדל
        const rawStage = stageMap[department];
        const stage = isValidStage(rawStage) ? rawStage : 'growing';

        for (const batch of batches) {
          const rating = buildDemoRating(batch, i);
          await t.run(
            `INSERT INTO batch_ratings (user_id, batch_id, stage, rating, points, submitted_at)
...
// כל האצוות (כולל מבוטלות) עם כמות דירוגים וממוצע
...
router.post('/batches', async (req, res) => {
  try {
    ...
    const name = String(req.body?.name || '').trim();
    if (!code || !name) {
      return res.status(400).json({ error: 'קוד ושם אצווה הם שדות חובה' });
    }
    const exists = await db.one('SELECT id FROM batches WHERE code = ?', [code]);
    if (exists) return res.status(409).json({ error: 'קוד אצווה זה כבר קיים במערכת' });

    const stage = isValidStage(String(req.body?.stage || '')) ? String(req.body.stage) : 'growing';
    const salesTarget = Number.isInteger(Number(req.body?.sales_target)) && req.body?.sales_target !== '' && req.body?.sales_target != null
      ? Number(req.body.sales_target) : null;

    const r = await db.run(`
      INSERT INTO batches (code, name, name_en, description, stage, sales_target, quarter, product_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      code,
      name,
      normalizeNullable(req.body?.name_en),
      normalizeNullable(req.body?.description),
      stage,
      salesTarget,
      normalizeNullable(req.body?.quarter),
      normalizeNullable(req.body?.product_id)
    ]);
    const batch = await db.one('SELECT * FROM batches WHERE id = ?', [r.insertId]);
...
// עדכון אצווה — כולל קידום שלב והפעלה/סגירה של הדירוג
router.patch('/batches/:id', async (req, res) => {
  try {
    ...
    const current = await db.one('SELECT * FROM batches WHERE id = ?', [id]);
    if (!current) return res.status(404).json({ error: 'האצווה לא נמצאה' });
    ...
    if (body.code !== undefined) {
      ...
      if (!code) return res.status(400).json({ error: 'קוד אצווה לא יכול להיות ריק' });
      const dup = await db.one('SELECT id FROM batches WHERE code = ? AND id <> ?', [code, id]);
      if (dup) return res.status(409).json({ error: 'קוד אצווה זה כבר קיים במערכת' });
      sets.push('code = ?'); params.push(code);
    }
    if (body.name !== undefined) {
      const name = String(body.name || '').trim();
      if (!name) return res.status(400).json({ error: 'שם אצווה לא יכול להיות ריק' });
      sets.push('name = ?'); params.push(name);
    }
    if (body.name_en !== undefined)     { sets.push('name_en = ?');     params.push(normalizeNullable(body.name_en)); }
    if (body.description !== undefined) { sets.push('description = ?'); params.push(normalizeNullable(body.description)); }
    if (body.image_url !== undefined)   { sets.push('image_url = ?');   params.push(normalizeNullable(body.image_url)); }
    if (body.product_id !== undefined)  { sets.push('product_id = ?');  params.push(normalizeNullable(body.product_id)); }
    if (body.quarter !== undefined)     { sets.push('quarter = ?');     params.push(normalizeNullable(body.quarter)); }
    if (body.stage !== undefined) {
      if (!isValidStage(String(body.stage))) return res.status(400).json({ error: 'שלב לא תקין' });
      sets.push('stage = ?'); params.push(String(body.stage));
    }
    if (body.sales_target !== undefined) {
      const st = body.sales_target === null || body.sales_target === '' ? null : Number(body.sales_target);
      if (st !== null && !Number.isInteger(st)) return res.status(400).json({ error: 'יעד מכירות לא תקין' });
      sets.push('sales_target = ?'); params.push(st);
    }
    if (body.rating_open !== undefined) {
      sets.push('rating_open = ?'); params.push(body.rating_open ? 1 : 0);
    }
    if (body.status !== undefined) {
      // מעבר ל-finished נעשה רק דרך הזנת תוצאה (POST /batches/:id/outcome)
      if (!['active', 'cancelled'].includes(String(body.status))) {
    ...
    params.push(id);
    await db.run(`UPDATE batches SET ${sets.join(', ')} WHERE id = ?`, params);

    const batch = await db.one('SELECT * FROM batches WHERE id = ?', [id]);
    res.json({ ok: true, batch });
  } catch (e) {
    console.error('admin/batches/update:', e);
    res.status(500).json({ error: 'שגיאת שרת' });
  }
});

// הזנת תוצאת אמת (1-5) — סוגרת את האצווה ומחלקת ניקוד
router.post('/batches/:id/outcome', async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!Number.isInteger(id) || id <= 0) {
      return res.status(400).json({ error: 'אצווה לא תקינה' });
    }
    const { outcome_level, sold_units } = req.body || {};
    if (!Number.isInteger(outcome_level) || outcome_level < 1 || outcome_level > 5) {
      return res.status(400).json({ error: 'תוצאה לא תקינה — יש לבחור רמה בין 1 ל-5' });
    }
    const soldUnits = sold_units === undefined || sold_units === null || sold_units === ''
      ? null : Number(sold_units);
    if (soldUnits !== null && !Number.isInteger(soldUnits)) {
      return res.status(400).json({ error: 'כמות יחידות שנמכרו לא תקינה' });
    }

    const batch = await db.one('SELECT * FROM batches WHERE id = ?', [id]);
    if (!batch) return res.status(404).json({ error: 'האצווה לא נמצאה' });

    // רבעון ברירת מחדל אם לא הוגדר — הרבעון הנוכחי (למשחק הרבעוני)
    await db.run(`
      UPDATE batches
      SET outcome_level = ?, sold_units = ?, status = 'finished', resolved_at = NOW(),
          rating_open = 0, quarter = COALESCE(quarter, ?)
      WHERE id = ?
    `, [outcome_level, soldUnits, currentQuarter(), id]);

    const updated = await recalcForBatch(id);
    res.json({ ok: true, ratings_updated: updated });
  } catch (e) {
    console.error('admin/batches/outcome:', e);
    res.status(500).json({ error: 'שגיאת שרת' });
  }
});

// ביטול תוצאה — האצווה חוזרת למצב פעיל והניקוד מאופס
router.delete('/batches/:id/outcome', async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!Number.isInteger(id) || id <= 0) {
      return res.status(400).json({ error: 'אצווה לא תקינה' });
    ...
    await db.run(`
      UPDATE batches
      SET outcome_level = NULL, sold_units = NULL, status = 'active', resolved_at = NULL
      WHERE id = ?
    `, [id]);
    await db.run('UPDATE batch_ratings SET points = 0 WHERE batch_id = ?', [id]);
    await db.run('UPDATE guess_group_bets SET points = 0 WHERE batch_id = ?', [id]);
    // TODO: להחזיר/לבטל הימורי מטבעות על האצווה כאשר services/coins.js יחשוף voidCoinBetsForBatch
    res.json({ ok: true });
  } catch (e) {
    console.error('admin/batches/clear-outcome:', e);
    res.status(500).json({ error: 'שגיאת שרת' });
  }
});

// מחיקת אצווה — רק אם אין עליה דירוגים
router.delete('/batches/:id', async (req, res) => {
  try {
    ...
    const batch = await db.one('SELECT id FROM batches WHERE id = ?', [id]);
    if (!batch) return res.status(404).json({ error: 'האצווה לא נמצאה' });

    const ratings = await db.one('SELECT COUNT(*) AS n FROM batch_ratings WHERE batch_id = ?', [id]);
    if (Number(ratings?.n || 0) > 0) {
      return res.status(409).json({ error: 'לא ניתן למחוק אצווה שכבר קיימים עליה דירוגים — ניתן לבטלה במקום' });
    }
    await db.run('DELETE FROM batches WHERE id = ?', [id]);
    res.json({ ok: true });
  } catch (e) {
    console.error('admin/batches/delete:', e);
    res.status(500).json({ error: 'שגיאת שרת' });
  }
});

// ריביוים של משתמש (לעריכה/השבתה מתוך מסך הניהול)
router.get('/users/:id/reviews', async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!Number.isInteger(id)) return res.status(400).json({ error: 'מזהה לא תקין' });
    const rows = await db.query(`
      SELECT r.id, r.batch_id, r.body, r.status, r.created_at,
...
router.patch('/reviews/:id', async (req, res) => {
  try {
    ...
    vals.push(id);
    const r = await db.run(`UPDATE batch_reviews SET ${sets.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, vals);
...
router.delete('/reviews/:id', async (req, res) => {
  try {
    const id = Number(req.params.id);
    const r = await db.run('DELETE FROM batch_reviews WHERE id = ?', [id]);
...
router.post('/send-emails', upload.array('attachments', 6), async (req, res) => {
  try {
    ...
    const userSenderName = 'אתר הצבעת אצווה - AtzvaVote';
...
// חישוב מחדש של כל הניקוד (כל האצוות שנפתרו)
router.post('/recalculate', async (req, res) => {
  try {
    const batches = await db.query("SELECT id FROM batches WHERE status = 'finished'");
    let total = 0;
    for (const b of batches) total += await recalcForBatch(b.id);
    res.json({ ok: true, ratings_updated: total });
  } catch (e) {
...
// כל הדירוגים של משתמש מסוים (לצפייה)
router.get('/users/:id/ratings', async (req, res) => {
  try {
    ...
    console.error('admin/user-ratings:', e);
    res.status(500).json({ error: 'שגיאת שרת' });
  }
});
