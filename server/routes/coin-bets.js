...
// ההימור: האם האצווה תצליח? success = תוצאה 4-5, fail = תוצאה 1-3
...
// ───────── הימורי ניחושים מיוחדים (למשל: אצוות הרבעון — market 'batch_top') ─────────
...
router.get('/mine', auth(), async (req, res) => {
  try {
    const rows = await db.query(`
      SELECT b.*, ba.code AS batch_code, ba.name AS batch_name, ba.name_en AS batch_name_en,
             ba.stage AS batch_stage, ba.status AS batch_status, ba.outcome_level,
...
router.get('/open', auth(), async (req, res) => {
  try {
    const rows = await db.query(`
      SELECT b.id, b.batch_id, b.proposition, b.stake, b.creator_id, b.target_user_id, b.created_at,
             b.max_acceptors, b.accepted_count,
             ba.code AS batch_code, ba.name AS batch_name, ba.name_en AS batch_name_en,
             ba.stage AS batch_stage, ba.status AS batch_status,
...
// ───────── מצא יריב: משתמשים שהדירוג שלהם בצד ההפוך של 4 (פתוחים לאתגור) ─────────
// הדירוג שלי 4-5 ("תצליח") ↔ הדירוג שלהם 1-3 ("תיכשל"), ולהפך
router.get('/opponents', auth(), async (req, res) => {
  try {
    const SIDE = "(CASE WHEN p.rating >= 4 THEN 'success' ELSE 'fail' END)";
    const rows = await db.query(`
      SELECT ba.id AS batch_id, ba.code AS batch_code, ba.name AS batch_name, ba.name_en AS batch_name_en,
             ba.stage AS batch_stage,
             myp.side AS my_prop,
             u.id AS user_id, u.name AS user_name, u.profile_image_url,
             op.side AS their_prop
      FROM (
        SELECT p.batch_id, ${SIDE} AS side
        FROM batch_ratings p
        WHERE p.user_id = ?
      ) myp
      JOIN batches ba ON ba.id = myp.batch_id AND ba.status = 'active'
      JOIN (
        SELECT p.user_id, p.batch_id, ${SIDE} AS side
        FROM batch_ratings p
      ) op ON op.batch_id = myp.batch_id AND op.side <> myp.side AND op.user_id <> ?
      ...
      ORDER BY ba.started_at DESC, u.name ASC
      LIMIT 200
...
router.post('/', auth(), async (req, res) => {
  try {
    ...
    if (batch.status !== 'active') return res.status(403).json({ error: 'ההימורים פתוחים רק על אצווה פעילה' });
    ...
    const betId = await db.tx(async (t) => {
      ...
      const r = await t.run(
        `INSERT INTO coin_bets (batch_id, proposition, stake, creator_id, target_user_id, max_acceptors, status)
         VALUES (?, ?, ?, ?, ?, ?, 'open')`,
        [batchId, proposition, stake, req.user.id, targetUserId, slots]
      );
      return r.insertId;
    });
...
router.post('/:id/accept', auth(), async (req, res) => {
  try {
    ...
    const batch = await db.one('SELECT * FROM batches WHERE id = ?', [bet.batch_id]);
    if (!batch || batch.status !== 'active') return res.status(403).json({ error: 'האצווה כבר אינה פתוחה להימורים' });
