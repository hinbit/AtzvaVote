...
export default function Profile() {
  ...
  useEffect(() => {
    api.get('/ratings/stats').then(r => setStats(r.data)).catch(() => setStats(null));
...
function StatisticsCard({ stats, t, showGroup = false }) {
  // הסטטיסטיקות האישיות מגיעות שטוחות מ-/ratings/stats; נתוני הקבוצות (אם קיימים) תחת group
  const ind = stats || {};
  ...
  return (
    <div className="stat-card" style={{ borderTop: '4px solid var(--gold)' }}>
      ...
      <div className="gg-stats-grid">
        ...
        <Metric value={ind.num_ratings ?? 0} label={t('profile.stat_ratings')} />
        <Metric value={ind.exact_hits ?? 0} label={t('profile.stat_exact')} />
        <Metric value={ind.close_hits ?? 0} label={t('profile.stat_close')} />
        <Metric value={ind.misses ?? 0} label={t('profile.stat_misses')} />
        <Metric value={ind.available_points ?? 0} label={t('profile.stat_available')} />
      </div>
