...
const touch = () => localStorage.setItem('atzva_last_active', String(Date.now()));
...
export function AuthProvider({ children }) {
  ...
  const [productsEnabled, setProductsEnabled] = useState(true); // רמה 2 — דירוג מוצרים חיצוני
  const [battlesEnabled, setBattlesEnabled] = useState(true); // קרבות השוואה ראש-בראש

  // דגלי תכונות ציבוריים
  useEffect(() => {
    api.get('/site/features')
      .then(r => {
        setCoinsEnabled(r.data?.coins_enabled !== false);
        setCoinsLeaderboardEnabled(r.data?.coins_leaderboard_enabled === true);
        setProductsEnabled(r.data?.products_enabled !== false);
        setBattlesEnabled(r.data?.battles_enabled !== false);
      })
      .catch(() => { setCoinsEnabled(true); setCoinsLeaderboardEnabled(false); setProductsEnabled(true); setBattlesEnabled(true); });
  }, []);

  const persistSession = (token, u) => {
    localStorage.setItem('atzva_token', token);
    touch();
    setUser(u);
  };
  ...
  useEffect(() => {
    ...
    const lastActive = Number(localStorage.getItem('atzva_last_active') || 0);
    if (!token) { setLoading(false); return; }
    if (lastActive && Date.now() - lastActive > INACTIVITY_MS) {
      localStorage.removeItem('atzva_token');
      localStorage.removeItem('atzva_last_active');
      setLoading(false);
      return;
    }
    api.get('/auth/me')
      .then(r => {
        if (r.data.token) localStorage.setItem('atzva_token', r.data.token);
        touch();
        setUser(r.data.user);
      })
      .catch(() => localStorage.removeItem('atzva_token'))
  ...
  return (
    <AuthContext.Provider value={{
      ...
      productsEnabled, battlesEnabled
    }}>
      {children}
