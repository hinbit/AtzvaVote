...
// רמות ניחוש אפשריות לתוצאת אצווה (1 = חלש, 5 = מצוין)
...
export default function GuessGroupDetail() {
  ...
  // add-bet state
  const [betBatch, setBetBatch] = useState('');
  const [betLevel, setBetLevel] = useState(3);
  ...
  useEffect(() => { load(); }, [id]);
  useEffect(() => { api.get('/batches').then(r => setBatches(r.data)).catch(() => setBatches([])); }, []);
  ...
  // אצוות זמינות לניחוש: פעילות בלבד, שטרם נוחשו על-ידי הקבוצה
  const betBatchIds = new Set(bets.map(b => b.batch_id));
  const availableBatches = batches.filter(b => b.status === 'active' && !betBatchIds.has(b.id));
  ...
  const addBet = async () => {
    setErr('');
    if (!betBatch) return;
    try { await api.post(`/guess-groups/${id}/bets`, { batch_id: Number(betBatch), guess_level: betLevel }); setBetBatch(''); load(); }
    catch (e) { setErr(errMsg(e)); }
  };
  const changeBet = async (batchId, level) => {
    setErr('');
    try { await api.post(`/guess-groups/${id}/bets`, { batch_id: batchId, guess_level: level }); load(); }
    catch (e) { setErr(errMsg(e)); }
  };
  const removeBet = async (batchId) => {
    setErr('');
    try { await api.delete(`/guess-groups/${id}/bets/${batchId}`); load(); }
    catch (e) { setErr(errMsg(e)); }
  };
  ...
  return (
    <main className="page">
      ...
      {/* Bets — ניחוש רמת ההצלחה הסופית (1-5) של כל אצווה */}
      <div className="stat-card" style={{ borderTop: '4px solid var(--gold)' }}>
        ...
        {can_manage && (
          <div className="gg-add-bet">
            <select value={betBatch} onChange={e => setBetBatch(e.target.value)}>
              <option value="">{t('gg.choose_batch')}</option>
              {availableBatches.map(b => (
                <option key={b.id} value={b.id}>
                  🌿 {pickText(b.name, b.name_en)} ({b.code}) · {stageDisplay(t, b.stage)}
                </option>
              ))}
            </select>
            <div className="gg-pick-group">
              {LEVELS.map(lv => (
                <button key={lv} type="button"
                  className={`gg-pick-btn ${betLevel === lv ? 'active' : ''}`}
                  onClick={() => setBetLevel(lv)}>{lv}</button>
              ))}
            </div>
            <button className="btn btn-gold" type="button" onClick={addBet} disabled={!betBatch}>{t('gg.add_bet')}</button>
          </div>
        )}
        ...
        {bets.length === 0 ? (
          ...
          <div className="table-wrap" style={{ marginTop: 12 }}>
            <table className="standings-table gg-bets-table">
              <thead>
                <tr>
                  <th style={{ textAlign: 'start' }}>{t('gg.batch')}</th>
              ...
              <tbody>
                {bets.map(b => {
                  // נעילה לפי סטטוס האצווה: ברגע שאינה פעילה — הניחוש נעול
                  const locked = b.status !== 'active';
                  const finished = b.status === 'finished' && b.outcome_level != null;
                  return (
                    <tr key={b.id}>
                      <td>
                        <div className="gg-bet-match">
                          <span>🌿 {pickText(b.batch_name, b.batch_name_en)}</span>
                          <span className="gg-vs">({b.batch_code})</span>
                          <span style={{ color: 'var(--muted)', fontSize: 12 }}>{stageDisplay(t, b.stage)}</span>
                        </div>
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {(can_manage && !locked) ? (
                          <div className="gg-pick-group gg-pick-inline">
                            {LEVELS.map(lv => (
                              <button key={lv} type="button"
                                className={`gg-pick-btn ${b.guess_level === lv ? 'active' : ''}`}
                                onClick={() => changeBet(b.batch_id, lv)}>{lv}</button>
                            ))}
                          </div>
                        ) : (
                          <span className="points-pill">{b.guess_level}</span>
                        )}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {finished ? <strong>{b.outcome_level}</strong> : <span style={{ color: 'var(--muted)' }}>{locked ? t('gg.pending') : '—'}</span>}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        {finished
                      ...
                      {can_manage && (
                        <td style={{ textAlign: 'center' }}>
                          {!locked && <button className="gg-remove-x" onClick={() => removeBet(b.batch_id)} title={t('gg.remove')}>×</button>}
                        </td>
                      )}
                    </tr>
                  );
